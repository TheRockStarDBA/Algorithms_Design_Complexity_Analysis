...
def get_shortest_path_for_this_state(goal_state_config, initial_state_config, state_space_dict):
    ...
    while len(FIFS_queue_to_traverse_in_BFS) != 0:
        ...
        for neighbor_config in (neighbors):
            ...
            state_space_dict[str(neighbor_config)] = (state_space_dict[str(state_to_check_config)][0] + state_space_dict[str(neighbor_config)][0], state_space_dict[str(neighbor_config)][1])

            if state_space_dict[str(neighbor_config)][1] == maxsize:
                FIFS_queue_to_traverse_in_BFS.append(neighbor_config)
                state_space_dict[str(neighbor_config)] = state_space_dict[str(neighbor_config)][0], state_space_dict[str(neighbor_config)][1] + 1
...
def ShortestPath(goal_state_config, list_of_initial_states_config):
    ...
    for initial_state_config in list_of_initial_states_config:
        ...
        shortest_path_for_this_state = get_shortest_path_for_this_state(goal_state_config, initial_state_config, deepcopy_of_state_space)
...
initial_states = [[1,2,3,4,5,6,8,7,0], [8, 3, 4, 0, 7, 1, 6, 2, 5]]
